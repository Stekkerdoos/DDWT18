 ...
 * @param array $template Array with as Key active page Id and as Value arrays containing name and corresponding url of page
 * @param string $active_id Id of active page
...
function update_serie($pdo, $serie_info){
    ...
    if ($serie_info['user'] != get_user_id()) {
        return [
            'type' => 'danger',
            'message' => 'There was an error. You are not the owner of this series.'
        ];
    }
 ...
 * @param object $pdo database object
 * @param string $user_id string containing id of user
...
/**
 * Checks if user exists and logs the user in when successful
 * @param object $pdo database object
 * @param array $form_data with POST data
 * @return array
 */
 ...
 * @return bool True if an active session exists else false
 */
